...
func sendAccessDenied(w http.ResponseWriter, detail string) {
...
func (proxy *Inkfish) mitmConnect(w http.ResponseWriter, req *http.Request) {
	...
	cconn, err := handshake(w, req, sConfig)
	if err != nil {
		log.Println("handshake", req.Host, err)
		return
	}
	defer cconn.Close()
...
// error if any.
func handshake(w http.ResponseWriter, r *http.Request, config *tls.Config) (net.Conn, error) {
